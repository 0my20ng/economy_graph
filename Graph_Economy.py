...
def simulate_market(ax, a, b, c, d, tax=0, subsidy=0, price_control_type=None, price_control=None):
    ...
    p_max = a / b
    cs_poly = Polygon([[0, p_max], [q_eq, p_eq], [0, p_eq]], closed=True, color='skyblue', alpha=0.3, label=f'소비자 잉여: {cs:.1f}')
    ax.add_patch(cs_poly)
    p_min = -c / d
    ps_poly = Polygon([[0, p_min], [q_eq, p_eq], [0, p_eq]], closed=True, color='lightgreen', alpha=0.3, label=f'생산자 잉여: {ps:.1f}')
...
def run_gui():
    def on_submit():
        try:
            ...
            if control_type == 'none':
                control_type_final = None
                control_value = None
            else:
                control_type_final = control_type
                control_value = float(control_price_entry.get())
            simulate_market(ax, vals['수요 절편 a'], vals['수요 기울기 b'],
                            vals['공급 절편 c'], vals['공급 기울기 d'],
                            vals['세금'], vals['보조금'], control_type_final, control_value)
            canvas.draw()
    ...
    # 통제유형 선택 Combobox
    tk.Label(root, text='통제유형').grid(row=6, column=0)
    ...
    # 통제가격 입력
    tk.Label(root, text='통제가격').grid(row=7, column=0)
